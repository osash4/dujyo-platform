...
fn main() {
    ...
    let config_data = format!(
        ...
        config.max_supply,
        config.distributions.iter()
            .map(|d| format!("  {}: {} DYO ({:.1}%)", d.purpose, d.amount, d.percentage))
            .collect::<Vec<_>>()
            .join("\n")
    );
    
    std::fs::write("dujyo_token_config.txt", config_data)
    ...
    println!("\n💾 Configuration saved to: dujyo_token_config.txt");
