...
pub fn search_routes_public() -> Router<AppState> {
    Router::new()
        .route("/", get(search_content_public))
}
...
/// GET /api/v1/search?q=query&type=music&limit=20 (Public version - no auth required)
/// Search for content, users, and playlists
async fn search_content_public(
    State(state): State<AppState>,
    Query(params): Query<HashMap<String, String>>,
) -> Result<Json<SearchResponse>, StatusCode> {
    let _user_id = ""; // No user ID for public search
    ...
    // Search in playlists (if query is long enough) - only public playlists
    if query.len() >= 3 {
        ...
        // ✅ FIX: Don't fail if playlists table doesn't exist (for MVP)
        match sqlx::query(&playlist_query)
            .bind(&search_pattern)
            ...
            .await
        {
            ...
            Err(e) => {
                // ✅ FIX: Log error but continue (table might not exist in MVP)
                eprintln!("⚠️ [search] Playlists table not available (skipping): {}", e);
            }
        }
    }
...
/// GET /api/v1/search?q=query&type=music&limit=20 (Protected version - requires JWT)
/// Search for content, users, and playlists
